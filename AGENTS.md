...
**PDF output is not in Inter.** `svglite` and `ragg` both consult the systemfonts registry, so SVG and PNG come out in the vendored faces. `cairo_pdf()` does not — it goes through fontconfig, finds nothing called Inter, and silently substitutes. Today that means the PDF is set in Bitstream Vera Sans: visibly heavier, wider, and wide enough that `title_overflow()`, which measures in Inter, no longer describes what the PDF actually lays out. Nothing warns. SVG is the documented printer deliverable and is correct, so this is a defect in a secondary format, but it is a silent one.
